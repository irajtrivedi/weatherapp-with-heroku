  ...
  if (cityName === undefined){
    return res.json({
      ...
      displayText: 'City Name Var Empty'
        ...
        let path = host + '/v1/public/yql?q=select item.condition from weather.forecast where woeid in (select woeid from geo.places(1) where text="' + cityName +'")&format=json';
        console.log(path);
        http.get(path, (responseFromAPI) => {
            ...
            responseFromAPI.on('data', function (chunk) {
                ...
                if(weather_info === undefined){
                    ...
                    let dataToSend = 'Weather in ' + cityName + ' is ' + Number(((weather_info.temp - 32) * (5/9))).toFixed(0) + '.';
                    return res.json({
                        speech: dataToSend,
